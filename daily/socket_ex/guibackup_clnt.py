...
class MyFrame(Frame):
    def __init__(self, root):
        ...
        self.back_src_var = StringVar()
        self.compress_var = BooleanVar()
        self.init_components()

    def init_components(self):
        proj_name = Label(self, text="Backup Client")
        ...
        src_label = Label(self, text='src to backup: ')
        src_label.grid(row=3)
        self.back_src = Entry(self, textvariable=self.back_src_var)
        self.back_src_var.set(os.path.abspath(r'D:\workspace\scripts\Oct'))
        self.back_src.grid(row=3, column=1)

        tar_label = Label(self, text='tar?')
        tar_label.grid(row=4)
        self.compress_on = Checkbutton(self, text='True', variable=self.compress_var, onvalue=1, offvalue=0)
        self.compress_on.grid(row=4, column=1)

        self.start_srv_btn = Button(self, text='Start', command=self.start_send)
        self.start_srv_btn.grid(row=5)

        self.exit_srv_btn = Button(self, text='Exit', command=self.root.destroy)
